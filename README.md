...
`CV_RESULT_CALLBACK_SECRET` de FastAPI. Generá otro valor distinto para
`JWT_SECRET_KEY` de FastAPI. Nunca subas los archivos `.env` a Git.
...
| `IA_API_TOKEN` | Dejalo vacío en esta versión. Django lo enviaría como `Authorization: Bearer ...`, pero la ruta actual de FastAPI no valida ese header ni emite tokens JWT. |
...
para obtener un JWT ni un token que deba colocarse en `IA_API_TOKEN`.
